...
const getJoke = async (categories = ['Any'], blacklist = []) => {
    try {
         ...
         const blacklistParam = blacklist.length > 0 ? `blacklistFlags=${blacklist.join(",")}&` : "";

         //add category
        const categoryParam = categories.join(","); 

        const joke = await fetch(`https://v2.jokeapi.dev/joke/${categoryParam}?${blacklistParam}format=txt`);
        if (joke.status !== 200) {
